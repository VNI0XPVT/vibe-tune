...
const SongList = ({ song, number, showAlbum = false }: Props) => {
    ...
    const QueueBtn = () => (
        <p
            ...
            title={isInPlaylist ? 'Remove from Queue' : 'Add to Queue'}
            onClick={e => {
        ...
        >
            {isInPlaylist ? (
                ...
                <PlusCircle className="size-4 md:size-5 opacity-15 group-hover:opacity-100 text-foreground group-hover:text-green-500" />
            )}
        </p>
    );

    const Duration = () => <p className="max-md:text-xs pr-1">{formatDuration(song.duration)}</p>;
    ...
    const SongWithAlbum = () => (
        <>
            <img src={song.image} className="border rounded-md size-10 md:size-11 " />
    ...
    return (
        <div
            ...
            className={cn(
                ...
                'text-sm  rounded-md text-muted-foreground hover:bg-muted hover:shadow-md transition-all duration-300',
                showAlbum ? 'p-1.5' : ''
                // 'bg-red-950'
            )}
